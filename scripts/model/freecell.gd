...
func try_to_move_any_to_foundation(last_moved_card: Card) -> bool:
	...
	var non_empty_piles = (tree.get_nodes_in_group(Groups.NON_EMPTY_CELLS) +
						   tree.get_nodes_in_group(Groups.NON_EMPTY_CASCADES))
	for pile in non_empty_piles:
		if (pile.top_card() == last_moved_card or
			pile.top_card().is_in_group(Groups.MOVING_CARDS) or
			pile.top_card().is_in_group(Groups.DRAGGING_CARDS)):
				continue
